...
public class Table{
    ...
    private Clock Player1 = new Clock(Alliance.Black);
    private Clock Player2 = new Clock(Alliance.White);
    ...
    private void buttonPanelSetUP(){
        ...
        try {
            Player1 = new Clock(Alliance.White);
            Player2 = new Clock(Alliance.Black);
    ...
    private class JTile extends JPanel{
        ...
        private void actions(){
            ...
            this.addMouseListener(new MouseAdapter() {
                    ...
                    if((board.getTile(panel.ID).isOccupied()==false&&haspiece)||(board.getTile(panel.ID).isOccupied()&&board.getTile(panel.ID).getPiece().getAlliance()!=board.getTurn()&&haspiece)){
                        ...
                        try {
                            ...
                            if(lost!=null &&lost instanceof King){
                                ...
                                try {
                                    extracted(lost.getAlliance(),false);
                                } catch (InterruptedException e1) {
                                    // TODO Auto-generated catch block
                                    e1.printStackTrace();
                                }
                ...
                private void extracted(Alliance lost, boolean timeisup) throws InterruptedException {
                                    if(timeisup){
                                        board = new Board();
                                        board.createBoard();
